...
void G01_TMR2_ISR(void){
   ...
   switch(feed_state){
      case g01:
         ...
         break;
      case g95:
         TMR2_LoadPeriodRegister((uint8_t)(46665*60/(2*33)*ustep*S/feed));
         break;
      default:
         break;
   }
...
void mover_2(float distancia){
   ...
   switch(feed_state){
      case g01:
         ustep=1;
         S=(((46665*60/(2*33)*ustep)*inverse_time_feed));
         while(((46665*60/(2*33)*ustep*inverse_time_feed)>255)){
            ustep=ustep<<1;
            if(ustep>16){
               ustep=16;
               break;
         ...
         pasos =(int) (distancia * 33 * ustep);
         setMicroStep(ustep);
         break;
      case g95:
         ustep=1;
         while((uint16_t)(46665*60/(2*33)*ustep*S*inverse_time_feed)>255){
            ustep=ustep<<1;
            if(ustep>16){
               ustep=16;
               break;
   ...
   TMR2_LoadPeriodRegister((uint8_t)(46665*60/(2*33)*ustep/feed));
   TMR2_StartTimer();
   return;
}
